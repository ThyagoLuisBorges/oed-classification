...
# Load a pretrained EfficientNet model, removing the classification head
...
for img_path in tqdm(image_paths, desc="Processing Images", unit="image"):
    ...
    file_names.append(img_path.split('/')[-1])  # Extract the file name from the path

# Create DataFrame with features, labels, and file names
df_efficientnet = pd.DataFrame(features)
df_efficientnet['label'] = labels
df_efficientnet['filename'] = file_names  # Add the file names